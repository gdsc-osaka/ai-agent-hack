...
export type GenerateProfile = (
  ...
  Profile[],
  CloudFunctionError | InvalidProfileError | DBInternalError
>;
...
const validateProfiles = (
  ...
  return Result.combine(profiles.map((profile) => validateProfile(profile)));
};
...
export const generateProfile =
  ...
  (audioFile: File) =>
    callCloudFunction("uploadAudio", audioFile)
      .map((cloudFunctionData) => cloudFunctionData.map(convertToDBProfile))
      .andThen((dbProfiles) => {
