...
import { fetchDBStaffByUserId, fetchDBStaffForStoreRoleById } from '../infra/staff-repo';
...
import { fetchDBStaffInvitationByEmailAndPending, insertDBStaffInvitation } from '../infra/staff-invitation-repo';
...
app.openapi(storesRoute.inviteStaffToStore, async (c) => {
  ...
  const res = await inviteStaffToStoreController(
    inviteStaffToStore(runTransaction, fetchDBStaffForStoreRoleById, fetchDBStoreByPublicId, fetchDBStaffInvitationByEmailAndPending, insertDBStaffInvitation)
