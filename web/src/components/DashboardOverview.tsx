...
export function DashboardOverview() {
  ...
  const getAverageSpend = () => {
    const total = mockCustomers.reduce(
      (sum, customer) => sum + customer.averageSpend,
      0
    );
    return Math.round(total / mockCustomers.length);
  };
  const getTotalVisits = () =>
    mockCustomers.reduce((sum, customer) => sum + customer.totalVisits, 0);
  const getVIPCustomers = () =>
    mockCustomers.filter((customer) => customer.visitFrequency === "VIP")
      .length;
          ...
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-green-500/50 transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white text-2xl font-bold">
                  ¥{getAverageSpend().toLocaleString()}
                </p>
