...
export const OrderHistory: React.FC<OrderHistoryProps> = ({ orders }) => {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'cocktail': return 'bg-amber-500/20 text-amber-400';
      case 'wine': return 'bg-red-500/20 text-red-400';
      case 'beer': return 'bg-yellow-500/20 text-yellow-400';
      case 'spirits': return 'bg-orange-500/20 text-orange-400';
      case 'non-alcoholic': return 'bg-blue-500/20 text-blue-400';
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
      ...
      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order.id} className="bg-gray-700/30 rounded-lg p-4 border border-gray-600">
